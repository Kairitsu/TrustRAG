...
|-------|------|------|--------|---------|
| #12 | Windows 端卸载/升级数据污染 | 部分完成 | 60% | InnoSetup 卸载器改进 |
| #13 | 新增 Rerank 重排模型配置 | 基本完成 | 95% | 文档说明 |
| #14 | 完善知识图谱功能 | 半完成 | 55% | 三层图谱/节点拖拽/人工编辑 |
| #15 | 优化引用编号视觉显示 | 基本完成 | 90% | 暗色模式适配 |
| #16 | 多账号数据隔离 | 基本完成 | 90% | 多账号同时在线体验 |
| #17 | 桌面端内置 PDF/DOCX 解析 | 部分完成 | 70% | OCR bin_path 修复/一键安装向导 |
...
- [ ] **13.10 UI 内说明文档**
  - **现状**: 前端没有 embedding vs rerank 的区别说明
  - **建议**: 在 Rerank 配置 Tab 顶部添加简短说明卡片
  - 低优先级，不影响功能
...
- [ ] **14.11 点击关系边查看来源证据**
  - **现状**: 点击节点有详情，但关系边的交互不完善
  - **需要**: 点击边 → 弹出关系详情（关系类型、来源文档、来源 chunk、原文片段、置信度）
  - **复杂度**: 中
...
- [ ] **15.4 暗色模式引用编号适配**
  - **现状**: 使用 `theme.colorScheme.primary` 和 `primaryContainer`，依赖主题但未专门测试暗色模式
  - **建议**: 确认暗色主题下引用编号对比度足够
  - **复杂度**: 低
...
- [ ] **16.7 多账号同时在线列表优化**
  - **现状**: 基础切换功能已有，但列表 UI 体验可优化
  - **建议**: 参考飞书/Slack 的账号切换体验，增加头像、最后活跃时间等
  - **复杂度**: 低
  - **优先级**: P3
...
| 编号 | 内容 | 预估工时 | Issue |
|------|------|---------|-------|
| 13.10 | Rerank 配置页 UI 说明文档 | 1h | #13 |
| 14.7 | 三层图谱架构 | 16h+ | #14 |
| 14.9 | 实体/关系人工编辑 | 12h+ | #14 |
| 14.11 | 点击关系边查看来源证据 | 3h | #14 |
| 15.4 | 暗色模式引用编号适配 | 1h | #15 |
| 16.7 | 多账号切换 UI 优化 | 2h | #16 |
| 17.9 | 一键 OCR 安装向导 | 16h+ | #17 |
| 17.10 | OCR 集成到文档处理管线 | 8h+ | #17 |
| 12.6 | 卸载程序命名优化 | 0.5h | #12 |
